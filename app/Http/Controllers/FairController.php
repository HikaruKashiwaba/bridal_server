    ...
    */
    ...
    public function getFair(string $memberId, string $fairId)
    {
        ...
        if ($fair->mynavi_flg == self::REGISTERED) {
            ...
            $fair->fair_mynavi->fair_content = FairContent::where('fair_id', $fairId)->where('site_type', self::FAIR_SITE_TYPE['MYNAVI'])->get();
        }
        //ぐるなびに登録してある場合
        if ($fair->gurunavi_flg == self::REGISTERED) {
            $fair->fair_gurunavi = FairGurunavi::where('fair_id', $fairId)->first();
            $fair->fair_gurunavi->fair_content = FairContent::where('fair_id', $fairId)->where('site_type', self::FAIR_SITE_TYPE['GURUNAVI'])->get();
        }
        //楽天に登録してある場合
        if ($fair->rakuten_flg == self::REGISTERED) {
            $fair->fair_rakuten = FairRakuten::where('fair_id', $fairId)->first();
            $fair->fair_rakuten->fair_content = FairContent::where('fair_id', $fairId)->where('site_type', self::FAIR_SITE_TYPE['RAKUTEN'])->get();
        }
        //みんなのウェディングに登録してある場合
    ...
    public function store(string $memberId, Request $request)
    {
        ...
        try {
            ...
            if ($params['zexy_flg'] == self::NEW_REGISTER || $params['zexy_flg'] == self::UPDATE_RECORD_REGISTERED) {
                ...
                $fair_zexy->part2 = $params['fair_zexy']['part2'];
                ...
                $fair_zexy->part3 = $params['fair_zexy']['part3'];
                ...
                $fair_zexy->part4 = $params['fair_zexy']['part4'];
                ...
                $fair_zexy->part5 = $params['fair_zexy']['part5'];
    ...
    public function update(string $memberId, string $fairId, Request $request) {
        ...
        try {
            ...
            for ($i = 0; $i < count(self::FAIR_FLG_NAME); $i++) {
                ...
                Log::debug('update '.self::FAIR_FLG_NAME[$i].' -->');
